...
describe("SDXLAdapter.emit", () => {
  it("emits a minimal Replicate SDXL payload", () => {
    expect(sdxlAdapter.emit(minimal)).toMatchInlineSnapshot(`
      {
        "payload": {
          "guidance_scale": 7.5,
          "height": 1024,
  ...
  it("emits the full cinematic-editorial prompt and payload", () => {
    expect(sdxlAdapter.emit(cinematicEditorial)).toMatchInlineSnapshot(`
      {
        "payload": {
          "guidance_scale": 10,
          "height": 1024,
  ...
  it("throws for required safety because SDXL exposes no canonical moderation control", () => {
    expect(() => sdxlAdapter.emit(standard, { criticality: { safety: "required" } })).toThrow(EmissionError);
  });

  it("warns and drops optional unsupported palette roles", () => {
    const out = sdxlAdapter.emit({ ...minimal, palette: { accent: "#c8943f", secondary: "#f5e6d3" } });
  ...
  it("is deterministic", () => {
    expect(JSON.stringify(sdxlAdapter.emit(standard))).toBe(JSON.stringify(sdxlAdapter.emit(standard)));
  });
});
